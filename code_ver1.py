...
async def parse_page(html):
    ...
    for game in soup.select(".search_result_row"):
        ...
        details = game.select_one(".search_capsule img")
        developer = details["alt"] if details else "Unknown"
        genres = game.select_one(".search_tags").get_text(strip=True).replace(",", ";")
        release_date = game.select_one(".search_released").get_text(strip=True) if game.select_one(".search_released") else "Unknown"
...
async def scrape_query(query, max_pages):
    ...
    async with aiohttp.ClientSession() as session:
        for page in range(1, max_pages + 1):
            ...
            if html:
                page_results = await parse_page(html)
